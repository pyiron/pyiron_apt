...
def transcoder_job(apt_file="Si.apt", rng_file="Si.RNG", jobid=1):
    from paraprobe_parmsetup.transcoder_config import (
        ParmsetupTranscoder,
        TranscodingTask,
    )
    ...
    transcoder_config = transcoder.load_reconstruction_and_ranging(
        recon_fpath=apt_file, range_fpath=rng_file, jobid=jobid
    )
...
def surfacer_job(transcoder_results, ranger_results, jobid=1):
    import os
    from paraprobe_parmsetup.surfacer_config import (
        ParmsetupSurfacer,
        SurfaceMeshingTask,
    )
    ...
    surfacer_config = surfacer.compute_convex_hull_edge_model(
        recon_fpath=transcoder_results, range_fpath=ranger_results, jobid=jobid
    )
    os.system(
        f"export OMP_NUM_THREADS=1 && mpiexec -n 1 paraprobe_surfacer {jobid} {surfacer_config}"
    )
...
def distancer_job(transcoder_results, ranger_results, surfacer_results, jobid=1):
    import os
    from paraprobe_parmsetup.distancer_config import (
        ParmsetupDistancer,
        PointToTriangleSetDistancing,
    )
    ...
    os.system(
        f"export OMP_NUM_THREADS=1 && mpiexec -n 1 paraprobe_distancer {jobid} {distancer_config}"
    )
...
def distancer_reporter(distancer_results, quantiles=[0.01, 0.50, 0.99], threshold=1.0):
...
def tessellator_job(transcoder_results, ranger_results, jobid=1):
    import os
    from paraprobe_parmsetup.tessellator_config import (
        ParmsetupTessellator,
        TessellationTask,
    )
    ...
    os.system(
        f"export OMP_NUM_THREADS=1 && mpiexec -n 1 paraprobe_tessellator {jobid} {tessellator_config}"
    )
...
def tessellator_reporter(
    tessellator_results, jobid=1, entry_id=1, quantile_based=True, height=500, width=500
):
    ...
    tessellator_plot = tessellator_report.get_cell_volume_cdf(
        task_id=1, quantile_based=quantile_based
    )
...
def get_center(transcoder_results):
    ...
    with h5py.File(transcoder_results, "r") as h5r:
        ...
        center = [0.0, 0.0, 0.0]  # np.zeros([3], np.float32)
...
def selector_job(
    transcoder_results, ranger_results, center, boxdims=[10.0, 10.0, 40.0], jobid=1
):
    from paraprobe_utils.primscontinuum import (
        RoiRotatedCuboid,
        RoiRotatedCylinder,
        RoiSphere,
    )
    ...
    task.flt.add_spatial_filter(
        primitive_list=[RoiRotatedCuboid(center=center, boxdims=boxdims)]
    )
    task.flt.add_evaporation_id_filter(
        lival=(0, 2, np.iinfo(np.uint32).max)
    )  # each second ion only
    selector.add_task(task)
    selector_config = selector.configure(jobid)
    os.system(
        f"export OMP_NUM_THREADS=1 && mpiexec -n 1 paraprobe_selector {jobid} {selector_config}"
    )
...
def spatstat_job(
    ...
    # either or
    task.set_knn(kth=1, binwidth=0.01, rmax=2.0)
    task.set_rdf(binwidth=0.01, rmax=2.0)
    spatstat.add_task(task)
    spatstat_config = spatstat.configure(jobid)
    os.system(
        f"export OMP_NUM_THREADS=1 && mpiexec -n 1 paraprobe_spatstat {jobid} {spatstat_config}"
    )
...
def spatstat_plot(spatstat_results, jobid=1, entry_id=1, task_id=1, rho=1):
    ...
    spatstat_report.get_rdf(
        1, normalizer=1.0 / rho
    )  # dont forget to make a proper estimate for rho!
    return Image(
        filename="PARAPROBE.Spatstat.Results.SimID.1.nxs.EntryId.1.TaskId.1.Knn.Pdf.png",
        width=500,
        height=500,
    )
...
def nanochem_job(
    ...
    task.load_ion_to_edge_distances(
        fpath=distancer_results, dset_name=f"/entry1/point_to_triangle/distance"
    )

    task.set_delocalization_input(method="compute")
    task.set_delocalization_normalization(
        method="composition"
    )  # normalize to atomic fraction (at.-%)
    task.set_delocalization_whitelist(
        method="resolve_element", nuclide_hash=[element], charge_state=[]
    )  # iso-surface defined by all atoms of (molecular) ions with Cr in it ...
    task.set_delocalization_gridresolutions(
        length=[voxel_edge_length]
    )  # nm, list of voxel edge length, for each length one analysis
    task.set_delocalization_kernel(
        sigma=[sigma], size=pixel_size
    )  # nm and pixel respectively
    task.set_delocalization_isosurfaces(
        phi=np.linspace(start=iso_start, stop=iso_stop, num=iso_num, endpoint=True)
    )  # isosurface starting at 4 at.-% in steps of 1 at.-% until 5 at.-%
    ...
    task.set_delocalization_edge_threshold(1.0)
    ...
    os.system(
        f"export OMP_NUM_THREADS=1 && mpiexec -n 1 paraprobe_nanochem {jobid} {nanochem_config}"
    )
...
@Workflow.wrap.as_function_node(
    "nanochem_VolOverIsoComposition", "nanochem_NumberOverIsoComposition"
)
def nanochem_reporter(nanochem_results, width=500, height=500):
    ...
    nanochem_report.get_isosurface_objects_volume_and_number_over_isovalue(
        deloc_task_id=1
    )
    ...
    return Image(filename=n_plot_voc, width=width, height=height), Image(
        filename=n_plot_noc, width=width, height=height
    )
